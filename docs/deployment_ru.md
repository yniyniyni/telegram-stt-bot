...
- Полишинг Gemini является опциональной сторонней обработкой. При `GEMINI_POLISH_ENABLED=true` подходящий текст расшифровки отправляется в Google Gemini API; установите `false`, чтобы отключить этот дополнительный путь обработки.
