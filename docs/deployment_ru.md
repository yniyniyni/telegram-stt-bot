...
# Установка пакетов строго по package-lock.json
npm ci
```
...
# Модель распознавания Deepgram (по умолчанию: nova-3)
...
# Контроль доступа
# Fail-closed по умолчанию: оставьте false, если вы не хотите явно разрешить все группы/супергруппы.
ALLOW_ALL_CHATS=false
# Список ID чатов Telegram через запятую, которым разрешено использовать бота
ALLOWED_CHATS=-100123456789,987654321

# Fail-closed по умолчанию: оставьте false, если вы не хотите явно разрешить всех пользователей в ЛС.
# Если false или переменная не задана, в ЛС могут писать только ALLOWED_USERS.
...
# Установите в 'true', чтобы включить улучшение текста через Gemini.
...
После каждого изменения зависимостей или кода снова выполните production-установку и сборку перед перезапуском службы:
```bash
...
`UMask=0077` делает файлы, созданные службой, доступными для чтения только пользователю службы. Это помогает защитить `.env`, логи, временные файлы и базу SQLite от других локальных пользователей.
...
- База SQLite по умолчанию хранит текст расшифровок в открытом виде. Используйте строгого владельца файлов, `UMask=0077`, ограниченные резервные копии и шифрование диска/файловой системы, если расшифровки могут содержать чувствительные данные.
- Полишинг Gemini является опциональной сторонней обработкой и по умолчанию отключен. При `GEMINI_POLISH_ENABLED=true` подходящий текст расшифровки отправляется в Google Gemini API; оставьте `false`, чтобы отключить этот дополнительный путь обработки.
